...
static int irq_transfer(struct atciic *iface)
{
	if (!wait_for_completion_timeout(&iface->complete, iface->adap.timeout)) {
		iface->result = -EIO;
		PRINTK(KERN_INFO
			"%s(%s) fail, addrhit 0x%x, cmpl 0x%x, %s %d, int 0x%x, status 0x%x\n",
...
/* Generic i2c manager transfer entrypoint. */
...
static void atciic_remove(struct platform_device *pdev)
{
