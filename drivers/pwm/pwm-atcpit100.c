...
#define PWM_READL(offset)	readl(ap->base + (offset))
			      ...
			      int duty_ns, int period_ns)
{
	...
	if (pc < PERIOD_COUNT_MIN || pc > PERIOD_COUNT_MAX ||
	    dc < DUTY_CYCLE_HIGH_MIN || dc > DUTY_CYCLE_HIGH_MAX) {
...
#else
#define atcpit_pwmc_suspend NULL
#define atcpit_pwmc_resume NULL
#endif
