...
static void andes_rproc_kick(struct rproc *rproc, int vqid)
{
	...
	writel(mask, local->atcsmu_base + MBOX_SET);
}
...
static irqreturn_t andes_remoteproc_interrupt(int irq, void *dev_id)
{
	...
	writel(mask, local->atcsmu_base + MBOX_CLR);
...
static int andes_rproc_start(struct rproc *rproc)
{
	 ...
	 */
	writel(rproc->bootaddr, local->atcsmu_base + SP_RESET_VEC_OFF);
	 ...
	 */
	reset = 0;
	reset = readl(local->atcsmu_base + RESET_N_OFF);
	if (reset & SP_CORE_BIT) {
		reset = reset ^ SP_CORE_BIT;
		writel(reset, local->atcsmu_base + RESET_N_OFF);
	}
...
static int andes_local_mem_access_port_release(struct rproc *rproc,
					       struct rproc_mem_entry *mem)
{
	memunmap(mem->va);
	return 0;
...
static int andes_local_mem_access_port_alloc(struct rproc *rproc,
					     struct rproc_mem_entry *mem)
{
	void *va;
...
static int andes_parse_local_mem_access_ports(struct rproc *rproc)
{
	struct device *dev = rproc->dev.parent;
	struct andes_rproc_pdata *local = rproc->priv;
	struct device_node *local_mem_access_port_nodes = dev->of_node;
	int i, num_local_mem_access_ports;

	num_local_mem_access_ports = of_count_phandle_with_args(local_mem_access_port_nodes,
								"local_mem_access_ports", NULL);
	if (num_local_mem_access_ports <= 0) {
		dev_err(dev, "need local memory access port to touch sp ILM/DLM\n");
		return -EINVAL;
	}

	local->num_local_mem_access_ports = num_local_mem_access_ports;

	for (i = 0; i < num_local_mem_access_ports; i++) {
		struct resource rsc;
		...
		dt_node = of_parse_phandle(local_mem_access_port_nodes, "local_mem_access_ports", i);
		...
		if (of_device_is_available(dt_node)) {
			...
			mem = rproc_mem_entry_init(dev, NULL, rsc.start,
						   (int)size, rsc.start,
						   andes_local_mem_access_port_alloc,
						   andes_local_mem_access_port_release,
						   rsc.name);
			if (!mem)
				return -ENOMEM;
...
static int andes_parse_fw(struct rproc *rproc, const struct firmware *fw)
{
	...
	ret = andes_parse_local_mem_access_ports(rproc);
	if (ret)
		return ret;
...
static int andes_remoteproc_probe(struct platform_device *pdev)
{
	...
	local->atcsmu_base = atcsmu_get_address();
	if (!local->atcsmu_base) {
		dev_err(&pdev->dev, "atcsmu get base is NULL or 0x0\n");
		goto error;
	}
